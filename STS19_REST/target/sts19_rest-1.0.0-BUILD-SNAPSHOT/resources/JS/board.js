...
function updateList(jsonObj){
	...
	if(jsonObj[0].status == "OK"){
		...
		var count = jsonObj[0].count;
		
		// 전역변수 업데이트!
		window.page = jsonObj[0].page;
		window.pageRows = jsonObj[0].pagerows;
		
		var i;
		var items = jsonObj[0].data;   // 배열
		for(i = 0; i < count; i++){
			result += "<tr>\n";
		...
		// 페이지 정보 업데이트
	$("#pageinfo").text(jsonObj[0].page + "/" + jsonObj[0].totalpage + "페이지, " + jsonObj[0].totalcnt + "개의 글");
		...
		// 페이징 업데이트
		var pagination = buildPagination(jsonObj[0].writepages, jsonObj[0].totalpage, jsonObj[0].page, jsonObj[0].pagerows);
...
function addViewEvent(){
	...
	$("#list .subject").click(function(){
		...
		// 읽어오기
		$.ajax({
			url : "list.ajax/" + $(this).attr('data-uid'),
			type : "GET",
			cache : false,
			success : function(data, status){
				// if(status == "success"){
