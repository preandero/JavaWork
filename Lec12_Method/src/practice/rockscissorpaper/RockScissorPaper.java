...
public class RockScissorPaper {
	...
	// method name: computeResult // User Win pattern 1,3 2,1 3,2
	// return: void
	// arguments:
	// 1) int user: 유저의 선택(가위(0)/바위(1)/보(2))
	// 2) int com: 컴퓨터의 선택(가위(0)/바위(1)/보(2)) // User lose pattern 1,2 2,3 3,1

	public static void computeResult(int user, int com) {
		if (user == com) {
			System.out.println("비겼습니다! ");
		}
		if (Integer.compare(user, com) == 1) {
			if (user + com == 4) {
				System.out.println("Com Win!");
			} else {
				System.out.println("User Win!");
			}
		} else if (Integer.compare(com, user) == 1) {
			if (user + com == 4) {
				System.out.println("User Win!");
			} else {
				System.out.println("Com Win!");
